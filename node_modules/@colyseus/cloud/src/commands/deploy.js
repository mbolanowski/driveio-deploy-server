...
export default async function deploy(options) {
  ...
  if (status.modified.length > 0) {
    ...
    console.log("");
    console.log(chalk.yellow("⚠️  You have these uncommited changes:"));
    console.log("");
  ...
  if (hasMultipleRemotes && env.remote !== remote) {
    ...
    console.log(`💾 Storing remote (${remote}) in ${config.COLYSEUS_CLOUD_JSON}`);
  ...
  try {
    ...
    if (e.data && e.data.error) {
      ...
      if (e.data.error.indexOf("token") >= 0) {
        console.log(`❗️ Consider deleting the '${config.COLYSEUS_CLOUD_JSON}' file and trying again (or provide --reset)`);
      }
      console.log("");

    } else if (e.message === "Service Unavailable") {
      console.log("");
      console.error(`⚠️  Colyseus Cloud is in maintenance. Please try again in a few minutes.`);
      console.log("");
